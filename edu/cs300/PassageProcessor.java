            ...
            while (true) {
                ...
                for (int i=0; i<workerList.size(); i++) {
                    ...
                    if (sendBack.contains("not found")) {
                        new MessageJNI().writeLongestWordResponseMsg(prefixCount, prefix, wID, workerList.get(wID).passageName, "----", workerList.size()-1, 0);
                    }
                    else {
                        new MessageJNI().writeLongestWordResponseMsg(prefixCount, prefix, wID, workerList.get(wID).passageName, sBthree[3].substring(1), workerList.size()-1, 1);
                    }
                }
